...
uint32_t nr_dlsch_decoding(PHY_VARS_NR_UE *phy_vars_ue,
  ...
  */
  ...
  LOG_I(PHY,"%d.%d DLSCH Decoding, harq_pid %d TBS %d (%d) G %d nb_re_dmrs %d length dmrs %d mcs %d Nl %d nb_symb_sch %d nb_rb %d\n",
  ...
  for (r=0; r<harq_process->C; r++) {
    ...
    if (err_flag == 0) {
      ...
      no_iteration_ldpc = nrLDPC_decod(p_decParams,
                                         (int8_t *)&pl[0],
                                         llrProcBuf,
                                         p_nrLDPC_procBuf[r],
                                         p_procTime);
  ...
  printf("PDSCH rcv offset %d, tbs %d ACK = %d \n", offset, harq_process->TBS, harq_process->ack);
  for(int ii = 0; ii < offset; ii++)
  {
     printf("%02x ", harq_process->b[ii]);
     if ((ii%32) == 31)
        printf("\n");
  }
  printf("\n");
  ...
  decode_sib1(harq_process->b,offset);
