...
static int is_css_config  = 0;
int frame_num_6hsb = (payload >> 25) & 0x3F;
 int scs_common = (payload >> 24) & 0x01;
 int ssb_scs_offset = (payload >> 20) & 0x0F;
 int dmrs_typeA_pos = ((payload >> 19) & 0x01) + 2;
 int config_sib1 = (payload >> 11) & 0xFF;
 uint16_t ssb_offset_point_a = (frame_parms->ssb_start_subcarrier - ssb_scs_offset)/12;
 ue->type0_PDCCH_CSS_config.dmrs_TypeA_Position = dmrs_typeA_pos;
 ue->type0_PDCCH_CSS_config.frame_num_6hsb = frame_num_6hsb;
 LOG_I(PHY, "pbch payload 0x%x, frame_num_6hsb %d, scs_common %d, ssb_scs_offset %d, dmrs_typeA_pos %d, config_sib1 %x, ssb_offset_point_a %d\n",
             payload, frame_num_6hsb, scs_common, ssb_scs_offset, dmrs_typeA_pos, config_sib1, ssb_offset_point_a);
 LOG_I(PHY, "frame_number_4lsb %d, half_frame_bit %d\n", frame_number_4lsb, frame_parms->half_frame_bit);
 
 if (is_css_config == 0)
 {
   proc->decoded_frame_rx = frame_number_4lsb;

 get_type0_PDCCH_CSS_config(&ue->type0_PDCCH_CSS_config,
                                      scs_common,
                                      frame_parms->ssb_index,
                                      config_sib1,
                                      kHz30, //frame_parms->subcarrier_spacing,
                                      (frequency_range_nr_t)frame_parms->freq_range,
