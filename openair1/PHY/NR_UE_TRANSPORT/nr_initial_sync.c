...
// #endif
  ...
  if (sa==1 && ret==0) {
    ...
    proc->nr_slot_rx = ue->type0_PDCCH_CSS_config.n_0;
    int pdcch_sc_id= (fp->first_carrier_offset + ue->type0_PDCCH_CSS_config.cset_start_rb *12) % fp->ofdm_symbol_size;
    LOG_I(PHY, "is %d pdccs frame %d %d search pdcch, firt sc offset %d, cset_start_rb %d, rb_offset %d, pdcch_sc_id %d, symbol %d %d\n", 
                      is, 
                      ue->type0_PDCCH_CSS_config.sfn_c,
                      ue->type0_PDCCH_CSS_config.n_0,
    ...
    for (uint16_t l=ue->type0_PDCCH_CSS_config.first_symbol_index; l < ue->type0_PDCCH_CSS_config.first_symbol_index + ue->type0_PDCCH_CSS_config.num_symbols; l++) 
    {
        nr_slot_fep_init_sync(ue,
                              proc,
                              l, // the UE PHY has no notion of the symbols to be monitored in the search space
                              ue->type0_PDCCH_CSS_config.n_0,
                              ((is + ue->type0_PDCCH_CSS_config.sfn_c) % 2)*fp->samples_per_frame+ue->rx_offset);
        short *temp_IQ=&ue->common_vars.common_vars_rx_data_per_thread[proc->thread_id].rxdataF[0][fp->ofdm_symbol_size*l];
        ...
        LOG_I(PHY, "pdcch_channel_estimation: symbol %d, first_carrier_offset %d, BWPStart %d, coreset_start_rb %d %d power %lf\n",
                l, fp->first_carrier_offset, ue->type0_PDCCH_CSS_config.cset_start_rb, ue->type0_PDCCH_CSS_config.rb_offset, ue->type0_PDCCH_CSS_config.num_rbs,temp_power_db);
        ...
        dci_cnt = nr_ue_pdcch_procedures(0, ue, proc, 0);
