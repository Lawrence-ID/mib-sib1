...
void nr_pdcch_channel_level(int32_t **dl_ch_estimates_ext,
  ...
  for (aarx=0; aarx<frame_parms->nb_antennas_rx; aarx++) {
    ...
    for (rb=0; rb<(nb_rb*3)>>2; rb++) {
      ...
      avg128P = vaddq_s32(avg128P,vcombine_s32(vpadd_s32(vget_low_s32(mmtmp0),vget_high_s32(mmtmp0)),vpadd_s32(vget_low_s32(mmtmp1),vget_high_s32(mmtmp1))));
...
#define NBR_RE_PER_RB_WITHOUT_DMRS         9
  ...
  for (aarx = 0; aarx < frame_parms->nb_antennas_rx; aarx++) {
    ...
    for (int rb=0;rb<coreset_nbr_rb;rb++) {

      c_rb = rb + coreset_start_rb;
      rxF=NULL;
...
int32_t nr_rx_pdcch_sib(PHY_VARS_NR_UE *ue,
  ...
  int32_t avgs;
  int32_t avgP[4];
  int n_rb,rb_offset, symb_start, symb_num;
  n_rb = ue->type0_PDCCH_CSS_config.num_rbs;
  rb_offset = 0;
  symb_start = ue->type0_PDCCH_CSS_config.first_symbol_index;
  symb_num = ue->type0_PDCCH_CSS_config.num_symbols;
  for (int s = symb_start; s < symb_start + symb_num; s++) {
    LOG_D(PHY,"in nr_pdcch_extract_rbs_single(rxdataF -> rxdataF_ext || dl_ch_estimates -> dl_ch_estimates_ext)\n");

    nr_pdcch_extract_rbs_sib(common_vars->common_vars_rx_data_per_thread[proc->thread_id].rxdataF,
...
#if 0
  LOG_I(PHY,"we enter nr_pdcch_demapping_deinterleaving(), symb_num %d, n_rb %d, number of candidates %d\n",symb_num, n_rb, ue->type0_PDCCH_CSS_config.number_of_candidates);
  LOG_I(PHY, "cce %d %d %d %d %d %d %d\n", ue->type0_PDCCH_CSS_config.CCE[0],
                                           ue->type0_PDCCH_CSS_config.CCE[1],
                                           ue->type0_PDCCH_CSS_config.CCE[2],
                                           ue->type0_PDCCH_CSS_config.CCE[3],
                                           ue->type0_PDCCH_CSS_config.CCE[4],
                                           ue->type0_PDCCH_CSS_config.CCE[5],
                                           ue->type0_PDCCH_CSS_config.CCE[6]);
  LOG_I(PHY, "L %d %d %d %d %d %d %d\n", ue->type0_PDCCH_CSS_config.L[0],
                                           ue->type0_PDCCH_CSS_config.L[1],
                                           ue->type0_PDCCH_CSS_config.L[2],
                                           ue->type0_PDCCH_CSS_config.L[3],
                                           ue->type0_PDCCH_CSS_config.L[4],
                                           ue->type0_PDCCH_CSS_config.L[5],
                                           ue->type0_PDCCH_CSS_config.L[6]);    
#endif                                       
  nr_pdcch_demapping_deinterleaving((uint32_t *) pdcch_vars->llr,
                                    ...
                                    2,
                                    0,
                                    ue->type0_PDCCH_CSS_config.number_of_candidates,
                                    ue->type0_PDCCH_CSS_config.CCE,
                                    ue->type0_PDCCH_CSS_config.L);
    /*
...
uint8_t nr_dci_decoding_procedure(PHY_VARS_NR_UE *ue,
  ...
  rnti_t n_rnti;
  pdcch_vars->mb = 65535;
  for (int j=0;j<ue->type0_PDCCH_CSS_config.number_of_candidates;j++) {
    int CCEind = ue->type0_PDCCH_CSS_config.CCE[j];
    int L = ue->type0_PDCCH_CSS_config.L[j];
    ...
    for (int k = 0; k < 1; k++) {
      ...
      int dci_length = ue->type0_PDCCH_CSS_config.dci_size;
      uint64_t dci_estimation[2]= {0};
      const t_nrPolar_params *currentPtrDCI = nr_polar_params(NR_POLAR_DCI_MESSAGE_TYPE, dci_length, L, 1, &ue->polarList);

//      LOG_I(PHY, "Trying DCI candidate %d of %d number of candidates, CCE %d (%d), L %d, length %d sirnti %d, nidcell %d\n",
//          j, ue->type0_PDCCH_CSS_config.number_of_candidates, CCEind, CCEind*9*6*2, L,
//            dci_length, SI_RNTI, ue->frame_parms.Nid_cell);
                                                
      nr_pdcch_unscrambling(&pdcch_vars->e_rx[CCEind*108], 0, L*108, ue->frame_parms.Nid_cell, tmp_e);
      ...
      n_rnti = SI_RNTI;
      //log_dump(PHY, tmp_e, 108, LOG_DUMP_SHORT, "tmp_e:");
      //LOG_I(PHY, "(%i.%i) dci indication (rnti %x,n_CCE %d,payloadSize %d, dci_estimation 0x %x %x, crc %d)\n",
      //      proc->frame_rx, proc->nr_slot_rx,n_rnti,CCEind,dci_length, dci_estimation[0], dci_estimation[1], crc);
      if (crc == n_rnti) {
        ...
        else
        {
          ...
          {
            pdcch_vars->mb = mb;
            pdcch_vars->dci_payload[0] = dci_estimation[0];
            pdcch_vars->dci_payload[1] = dci_estimation[1];
          }
        }

      } else {
        LOG_D(PHY,"(%i.%i) Decoded crc %x does not match rnti %x\n", proc->frame_rx, proc->nr_slot_rx, crc, n_rnti);
      }
    }
  }
  return (pdcch_vars->mb == 65535)? 0:1;
}
