...
#define _mm_subs_pi16(a,b) vqsub_s16(a,b)
#endif
...
void applyFtoleft(const t_nrPolar_params *pp, decoder_node_t *node) {
  ...
  if (node->left->all_frozen == 0) {
    ...
    if (avx2mod == 0) {
      ...
      int16x8_t a128,b128,absa128,absb128,minabs128;
      int avx2len = node->Nv/2/8;

      //      printf("avx2len %d\n",avx2len);
      for (int i=0;i<avx2len;i++) {
        a128       =((int16x8_t*)alpha_v)[i];
        b128       =((int16x8_t*)alpha_v)[i+avx2len];
        absa128    =_mm_abs_epi16(a128);
        absb128   =_mm_abs_epi16(b128);
        minabs128  =_mm_min_epi16(absa128,absb128);
        ...
        ((int16x8_t*)alpha_l)[i] =vorrq_s16(vandq_s16(minabs128,comp2),vandq_s16(negminabs128,comp1));
      ...
      int16x8_t zero=vdupq_n_s16(0);
      int16x8_t a128,b128,absa128,absb128,minabs128;
      a128       =*((int16x8_t*)alpha_v);
      b128       =((int16x8_t*)alpha_v)[1];
      absa128    =_mm_abs_epi16(a128);
      absb128    =_mm_abs_epi16(b128);
      minabs128  =_mm_min_epi16(absa128,absb128);
      signatimesb=veorq_s16(a128,b128);
      ...
      comp2=(int16x8_t)vcgeq_s16(signatimesb,zero);
      negminabs128=vnegq_s16(minabs128);
      *((int16x8_t*)alpha_l) =vorrq_s16(vandq_s16(minabs128,comp2),vandq_s16(negminabs128,comp1));
    }
    else if (avx2mod == 4) {
      int16x4_t a64,b64,absa64,absb64,minabs64;
      int16x4_t signatimesb,comp1,comp2,negminabs128;
      ...
      absb64    =_mm_abs_pi16(b64);
      minabs64  =_mm_min_pi16(absa64,absb64);
      signatimesb=veor_s16(a64,b64);
      comp1=(int16x4_t)vclt_s16(signatimesb,zero);
      comp2=(int16x4_t)vcge_s16(signatimesb,zero);
      negminabs128=vneg_s16(minabs64);
      *((int16x4_t*)alpha_l) =vorr_s16(vand_s16(minabs64,comp2),vand_s16(negminabs128,comp1));
    }
    else
#else
...
#elif defined(__arm__) || defined(__aarch64__)
    int arm2mod = (node->Nv/2)&15;
    if (arm2mod == 0) {
      int avx2len = node->Nv/2/16;
      int16x8_t sgn_tmp;
      for (int i=0;i<avx2len;i++) {
        sgn_tmp = vaddq_s16((int16x8_t)vcgtq_s16(vdupq_n_s16(0),((int16x8_t *)betal)[2*i]),(int16x8_t)vshrq_n_u16(vcltq_s16(vdupq_n_s16(0),((int16x8_t *)betal)[2*i]),15));
        sgn_tmp = vmulq_s16(((int16x8_t *)alpha_v)[2*i],sgn_tmp);
        ((int16x8_t *)alpha_r)[2*i] = vqsubq_s16(((int16x8_t *)alpha_v)[(avx2len*2)+2*i],sgn_tmp);

        sgn_tmp = vaddq_s16((int16x8_t)vcgtq_s16(vdupq_n_s16(0),((int16x8_t *)betal)[2*i+1]),(int16x8_t)vshrq_n_u16(vcltq_s16(vdupq_n_s16(0),((int16x8_t *)betal)[2*i+1]),15));
        sgn_tmp = vmulq_s16(((int16x8_t *)alpha_v)[2*i+1],sgn_tmp);
        ((int16x8_t *)alpha_r)[2*i+1] = vqsubq_s16(((int16x8_t *)alpha_v)[(avx2len*2)+2*i+1],sgn_tmp);
      }
    }
    else if (arm2mod == 8) {
      int16x8_t sgn_tmp;
      sgn_tmp = vaddq_s16((int16x8_t)vcgtq_s16(vdupq_n_s16(0),((int16x8_t *)betal)[0]),(int16x8_t)vshrq_n_u16(vcltq_s16(vdupq_n_s16(0),((int16x8_t *)betal)[0]),15));
      sgn_tmp = vmulq_s16(((int16x8_t *)alpha_v)[0],sgn_tmp);
      ((int16x8_t *)alpha_r)[0] = vqsubq_s16(((int16x8_t *)alpha_v)[1],sgn_tmp);
    }
    else if (arm2mod == 4) {
      int16x4_t sgn_tmp;
      sgn_tmp = vadd_s16((int16x4_t)vcgt_s16(vdup_n_s16(0),((int16x4_t *)betal)[0]),(int16x4_t)vshr_n_u16(vclt_s16(vdup_n_s16(0),((int16x4_t *)betal)[0]),15));
      sgn_tmp = vmul_s16(((int16x4_t *)alpha_v)[0],sgn_tmp);
      ((int16x4_t *)alpha_r)[0] = vqsub_s16(((int16x4_t *)alpha_v)[1],sgn_tmp);
    }
    else
#else
    int sse4mod = (node->Nv/2)&7;
...
#elif defined(__arm__) || defined(__aarch64__)
  if (node->left->all_frozen==0) { // if left node is not aggregation of frozen bits
    int arm4mod = (node->Nv/2)&15;
    register int16x8_t allones=*((int16x8_t*)all1);
    if (arm4mod == 0) {
      int armlen = node->Nv/2/8;
      for (int i=0;i<armlen;i++) {
        ((int16x8_t*)betav)[i] = vorrq_s16((int16x8_t)vceqq_s16 (((int16x8_t*)betar)[i],
                                               ((int16x8_t*)betal)[i]),allones);
      }
    }
    else if (arm4mod == 4) {
      ((int16x4_t*)betav)[0] = vorr_s16((int16x4_t)vceq_s16(((int16x4_t*)betar)[0],
                                                      ((int16x4_t*)betal)[0]),*((int16x4_t*)all1));
    }
    else if (arm4mod == 8) {
      ((int16x8_t*)betav)[0] = vorrq_s16((int16x8_t)vceqq_s16(((int16x8_t*)betar)[0],
                                         ((int16x8_t*)betal)[0]),*((int16x8_t*)all1));
    }
    else
    {
      for (int i=0;i<node->Nv/2;i++) {
