			 ...
			 int madd)
{
  ...
  for(i=0; i<(N>>2); i++) {
    ...
    tmp_im  = vmull_s16(vrev32_s16(vmul_s16(((simdshort_q15_t*)x1_128)[0],*(simdshort_q15_t*)conjug)), ((simdshort_q15_t*)x2_128)[0]);
    //tmp_im = [-Im(ch[0])Re(rx[0]) Re(ch[0])Im(rx[0]) -Im(ch[1])Re(rx[1]) Re(ch[1])Im(rx[1])]
    tmp_im1 = vmull_s16(vrev32_s16(vmul_s16(((simdshort_q15_t*)x1_128)[1],*(simdshort_q15_t*)conjug)), ((simdshort_q15_t*)x2_128)[1]);
