...
typedef struct {
  ...
  uint16_t sfn;
  uint16_t slot;
  uint64_t dci_payload[2];
  uint16_t mb; // mis bits
  /*
#ifdef NR_PDCCH_DEFS_NR_UE
  int nb_searchSpaces;
...
typedef enum {
  SFN_C_MOD_2_EQ_0_NEW,
  SFN_C_MOD_2_EQ_1_NEW,
  SFN_C_IMPOSSIBLE_NEW
} SFN_C_TYPE_NEW;
typedef enum {
  NON_INTERLEAVED,
  INTERLEAVED
} INTERLEAVED_TYPE;
typedef enum frequency_range_nr {
  NR_FR1 = 0,
  NR_FR2
} frequency_range_nr_t;

typedef enum common_scs_spacing {
  SCS15_OR_60 = 0,
  SCS30_OR_120
} common_scs_spacing;

typedef enum channel_bandwidth_new_e {
  bwnew_5MHz   = 0x1,
  bwnew_10MHz  = 0x2,
  bwnew_20MHz  = 0x4,
  bwnew_40MHz  = 0x8,
...
typedef struct Type0_PDCCH_CSS_config_new_s {
  int frame_num_6hsb;
  int32_t num_rbs;
  int32_t num_symbols;
  ...
  nr_scs_e scs_pdcch;
  INTERLEAVED_TYPE interLeaved_type; // INTERLEAVED
  uint32_t      reg_BundleSize;      // 6
  uint32_t      interleaverSize;     // 2
  //uint32_t      shiftIndex;
  ...
  bool active;
} NR_Type0_PDCCH_CSS_config_new_t;
...
typedef struct {
  ...
  NR_UE_PDSCH     *pdsch_vars[RX_NB_TH_MAX][NUMBER_OF_CONNECTED_gNB_MAX]; // two RxTx Threads
  ...
  //   *ulsch[RX_NB_TH_MAX][NUMBER_OF_CONNECTED_gNB_MAX][NR_MAX_NB_CODEWORDS]; // two code words
  ...
  /// PDSCH DMRS
  uint32_t ****nr_gold_pdsch[NUMBER_OF_CONNECTED_gNB_MAX];

  // Scrambling IDs used in PDSCH DMRS
  uint16_t scramblingID[2];

  /// PDCCH DMRS
  uint32_t ***nr_gold_pdcch[NUMBER_OF_CONNECTED_gNB_MAX];
...
// #endif
  ...
  int dl_stats[5];
  NR_Type0_PDCCH_CSS_config_new_t type0_PDCCH_CSS_config;
} PHY_VARS_NR_UE;
